...
def orfFinder(seq, trans_table, min_protein_length, proSeq):
	indices = []
		...
		for frame in range(3):											# 3 reading frames for each
			...
			while aa_start < trans_len:
				...
				if aa_end-aa_start == min_protein_length:	# If this is the same number of AA's as the Protein
					if pID(trans[aa_start:aa_end].lower(),proSeq.lower()) > 97:	# If there is greater than 97 % ID
						...
						# indices.append(start, end, strand,
...
def getSeqID(uniprotID):
	...
	if len(quadTuple) != 0:
		...
		startAndStop = orfFinder(dna_seq, table, min_pro_len, proSeq)	# Subroutine calls to determine
