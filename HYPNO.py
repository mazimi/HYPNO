...
def main():
	...
	parser.add_argument('--k', default = 90.0, type = float, help='Subtree selection: the “subtree selection” fractional sequence identity cutoff as input to the Kerf algorithm (default: 90).  To override the default setting, type “--k <X>”, where X is a real number between 0 and 100. For example, to set the subtree selection value to 93, you’d type “--k 93.0”. (default: 90.0)', required=False)
	parser.add_argument('--n', default = 95.0, type = float, help='PercentID match: the minimum fractional sequence identity that the translation of a retrieved nucleotide sequence must have relative to the expected protein sequence for it to be accepted (default: 95).  To override the default setting, type “--n <X>”, where X is a real number between 0 and 100. For example, to set the percentID match value to 80, you’d type “--n 80.0”. (default: 95.0)', required=False)
	parser.add_argument('--s', default = 100.0, type = float, help='Retrieval minimum: the minimum fraction of nucleotide sequences that must be retrieved in order for HYPNO to continue (default: 100). In the case that a retrieval minimum of less than 100% is specified and HYPNO encounters a protein sequence for which a nucleotide sequence with sufficient sequence identity is unavailable, HYPNO continues executing and excludes the sequence(s) from the final nucleotide MSA and tree outputs. To override the default setting, type “--s <X>”, where X is a real number between 0 and 100. For example, to set the retrieval minimum value to 90, you’d type “--s 90.0”. (default: 100.0)', required=False)
