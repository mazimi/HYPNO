...
def main():
	...
	segDict = {}
	with open('segMapping.tsv','r') as segFh:
		for segLine in segFh:
			match = re.search(r'^([^\t]+)\t[^\t]+\t([^\t]+)', segLine)
			if match:
				segDict[match.group(1)] = match.group(2)
			else:
				continue
	...
	for line in fin:
		...
		if match:
			...
			matchUniprot = re.search(r'^([A-N,R-Z][0-9][A-Z][A-Z,0-9][A-Z,0-9][0-9]|[O-Q][0-9][A-Z,0-9][A-Z,0-9][A-Z,0-9][0-9])', match.group(3))
			if matchUniprot:
				fout.write(match.group(3) + '\t' + match.group(1) + '\n')
			else:
				proteinID = match.group(2)
				if proteinID in segDict:
					fout.write(match.group(3) + '\t' + segDict[proteinID] + '\n')
		else:
			pass
		numLine += 1
